...
1. 思想：(分治) 不稳定
   ...
   2. 调整区间  ： 左边 区间<= x 右边区间>=x  (最难的一步).  i指针后面的数一定大于等于 x  j指针前面的数一定小于等于x
   3. 递归处理两端
2. 算法步骤
   1. 边界值递归退出
   2. 初始化分界点， i，j 指针
...
1. dfs和bfs的对比
   ...
   | ---- | -------- | --------- | ------------------- | ----------------------------- |
   | dfs  | stack    | O(h)      |                     |                               |
   | bfs  | queue    | o(2h次方) | 最短路(路径权值为1) | 最短路，最小次数，经常使用bfs |
...
1. 不是所有的最短路径都可以使用bfs，一般只有权重为1时才可以
2. 代码框架
   1. queue <- 初始状态
   2. while queue 不空
      1. 取出队头
